...
const sudaniInfo = {
    ...
    codes: {
        balance: '*222#',
        sah: '*500#',
        internet: '*4#',        // تم التحديث
        calls: '*6#',           // تم التحديث (كود المكالمات)
        unix: '*6#',            // نظام يونكس
        prepaid: '*4#',         // تم التحديث من *444# إلى *4#
        transfer: '*303#',
        lte: '*4*400#',
    },
    
    // ============================================
    // 1. باقات المكالمات (ريح بالك - أحلى يوم - خلي عنك)
    // ============================================
    callPackages: {
        rihBalak: {
...
function getLocalResponse(message) {
    ...
    if (msg.includes('يونكس') || msg.includes('unix') || msg.includes('نظام يونكس')) {
        return `📱 **نظام يونكس (UNIX) من سوداني:**\n\n` +
               ...
               `• 1 وحدة = 1 رسالة نصية\n\n` +
               `📱 **كيف تشترك؟**\n` +
               `• عبر الكود: **${sudaniInfo.codes.unix}**\n` +
               `• أو عبر تطبيق MySudani\n\n` +
    ...
    if (msg.includes('ريح بالك') || msg.includes('ريح')) {
        return `📞 **باقات ريح بالك من سوداني:**\n\n` +
               ...
               `📱 للاطلاع على العروض الحالية: اتصل بـ *4# أو *6#`;
    }
